...
try:
    ...
    #擷取需要片段
    payoff1 = rawdata1[rawdata1.loc[:, "payoff"] <= 0]
    payoff2 = rawdata2[rawdata2.loc[:, "payoff"] <= 0]
    payoff3 = rawdata3[rawdata3.loc[:, "payoff"] <= 0]
    payoff4 = rawdata4[rawdata4.loc[:, "payoff"] <= 0]
    payoff5 = rawdata5[rawdata5.loc[:, "payoff"] <= 0]
    #plot
    fig, axes = plt.subplots(nrows=2, ncols=3)
    plt.suptitle('Probability Densities on [-1,0]')
